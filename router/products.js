...
router.route("/add").post(async (req, res) => {
  ...
  thumbIGMURL = `${name.replace(/\s/g, "").replaceAll("/", "_") + "_main"}.${
    thumbIMG.split("/")[1].split(";")[0]
  }`;
  require("fs").writeFile(
    `${__dirname}/${name.replace(/\s/g, "").replaceAll("/", "_") + "_main"}.${
      thumbIMG.split("/")[1].split(";")[0]
    }`,
    base64Data,
    "base64",
  ...
  productIMG.map((IMGURL, i) => {
    ...
    imgARR.push({
      url: `${name.replace(/\s/g, "").replaceAll("/", "_") + i}.${
        IMGURL.split("/")[1].split(";")[0]
      }`,
    });
    require("fs").writeFile(
      `${__dirname}/${name.replace(/\s/g, "").replaceAll("/", "_") + i}.${
        IMGURL.split("/")[1].split(";")[0]
      }`,
      base64Data,
      "base64",
